...
require_once "../../Admin/Model/Admissionsmodel.php";
    ...
    class DBfees
    {
      public static function insert($feesObj)
      {
        ...
        if($feesObj->get_duedate()==""){
          $sql = "insert into fees (`Admissionid`,`Courseid`,`TotalFees`, `PaidFees`,`PendingFees`, `Feesplan`,`PaymentMode`,`PaymentDescription`) 
                values ('".$feesObj->get_admitid()."','".$feesObj->get_courseid()."','".$feesObj->get_tfees()."','".$feesObj->get_pfees()."','".$feesObj->get_pendingfees()."','".$feesObj->get_feesplan()."','".$feesObj->get_pmode()."','".$feesObj->get_pdescription()."')";
    ...
    public static function collectionoffees($viewObj)
    {
      ...
      $connectionObj=$db->getConnection();
         ...
         $sql="Select A.id, A.Courseid, Name, CoursesOpted , TotalFees, SUM(PaidFees) as PaidFees, SUM(PendingFees) as PendingFees from admissions as A 
        ...
        public static function viewfeesdetails($viewObj)
    {
      $db=ConnectDb::getInstance();
      $connectionObj=$db->getConnection();
       $sql = "select Modified_Date,Admissionid,PaidFees,PendingFees,PaymentMode from fees where Admissionid=$viewObj";
       $result = mysqli_query($db->getConnection(), $sql);
       $feesdetails=[];
       if (mysqli_num_rows($result) > 0) {
        while($row = mysqli_fetch_assoc($result)) {
